...
import { likeMovie } from "./likeAdd.js";
import { likeCheck } from "./likeCheck.js";
import { getMovieData, htmlGenerator } from "./util.js";
...
export async function showMovieDetails(e, movieIdent) {
    ...
    const movieLikes = await getMovieData(`http://localhost:3030/data/likes?where=movieId%3D%22${movie._id}%22&distinct=_ownerId&count`);
...
async function constructMovieDetailsPage(movie, numberOfLikes) {
    ...
    if (userId === ownerId) {
    ...
    } else if (await likeCheck(movieId, userId)) {
        ...
        likeBtn.addEventListener('click', (e) => likeMovie(e, movieId));
    }
